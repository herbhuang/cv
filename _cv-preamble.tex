...
%
% EB Garamond carries the body, the title name, and own-name marks in
% reference entries (tabular lining figures: years and dates sit on the
...
% Native names first: both Alegreya Sans and EB Garamond ship `medium`;
% `sb` is Alegreya's alias for that face (it has no SemiBold) and EB
% Garamond's real SemiBold. The fd must already be loaded -- \sffamily /
% \rmfamily / the document default all do that before we run.
...
% Own-name emphasis in reference lists: EB Garamond Medium, else SemiBold.
...
% Request the native `medium` face; fall back to `sb` (which Alegreya aliases
% to that same face) if a truncated install has no Medium Italic.

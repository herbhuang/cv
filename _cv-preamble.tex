...
% ---------------------------------------------------------------------------
% Typefaces. Two independent switches, both overridable from an entry file
% before \input{_cv-preamble}:
%   \def\cvfont{...}      body:     garamond | pagella | palatino | libertine
%                                   | libertinus
%   \def\cvheadfont{...}  headings: rm | heros | gillius | biolinum
%                                   | libertinussans | ysabeau
% `rm` puts the headings in the body serif, set bold. Packages load first and
% \rmdefault/\sfdefault are pinned afterwards, because several of these claim
% both families on load (libertine and libertinus set serif *and* sans).
% Classico/URW Optima is not in TeX Live's repository; gillius (Gill Sans ADF)
% is the nearest free humanist stand-in.
% ---------------------------------------------------------------------------
\ifx\cvfont\undefined\def\cvfont{libertinus}\fi
\ifx\cvheadfont\undefined\def\cvheadfont{libertinussans}\fi

\def\cvbodypagella{pagella}
\def\cvbodypalatino{palatino}
\def\cvbodylibertine{libertine}
\def\cvbodylibertinus{libertinus}
\ifx\cvfont\cvbodypagella
  \usepackage{tgpagella}\def\cvrmfam{qpl}
\else\ifx\cvfont\cvbodypalatino
  \usepackage{mathpazo}\def\cvrmfam{ppl}
\else\ifx\cvfont\cvbodylibertine
  \usepackage{libertine}\def\cvrmfam{LinuxLibertineT-TLF}
\else\ifx\cvfont\cvbodylibertinus
  \usepackage{libertinus-type1}\def\cvrmfam{LibertinusSerif-TLF}
\else
  \usepackage{ebgaramond}\def\cvrmfam{EBGaramond-OsF}
\fi\fi\fi\fi

\def\cvheadrm{rm}
\def\cvheadheros{heros}
\def\cvheadgillius{gillius}
\def\cvheadbiolinum{biolinum}
\def\cvheadlibsans{libertinussans}
\def\cvheadysabeau{ysabeau}
\ifx\cvheadfont\cvheadheros
  \usepackage{tgheros}\def\cvsffam{qhv}
\else\ifx\cvheadfont\cvheadgillius
  \usepackage{gillius}\def\cvsffam{GilliusADF-LF}
\else\ifx\cvheadfont\cvheadbiolinum
  % the libertine bundle already ships Biolinum; loading both clashes
  \ifx\cvfont\cvbodylibertine\else\usepackage{biolinum}\fi
  \def\cvsffam{LinuxBiolinumT-TLF}
\else\ifx\cvheadfont\cvheadlibsans
  \usepackage{libertinus-type1}\def\cvsffam{LibertinusSans-TLF}
\else\ifx\cvheadfont\cvheadysabeau
  \usepackage{ysabeau}\def\cvsffam{Ysabeau-TLF}
\else
  \def\cvsffam{\sfdefault}
\fi\fi\fi\fi\fi

\renewcommand*{\rmdefault}{\cvrmfam}
\ifx\cvheadfont\cvheadrm
  \newcommand{\cvheadfamily}{\rmfamily}
  \newcommand{\cvheaddefault}{\rmdefault}
\else
  \renewcommand*{\sfdefault}{\cvsffam}
  \newcommand{\cvheadfamily}{\sffamily}
  \newcommand{\cvheaddefault}{\sfdefault}
\fi

% Semibold italic for level-3 headings, from a real face wherever one exists.
% Ysabeau has one in the sans. Libertinus has one only in the SERIF: its sans
% ships no semibold and no bold italic at all (see T1LibertinusSans-TLF.fd),
% so level 3 steps down into the body face there -- which reads correctly as a
% level below a sans level 2. Anything else strokes the regular outline.
\ifx\cvheadfont\cvheadysabeau
  \newcommand{\cvsemi}[1]{{\fontseries{sb}\selectfont #1}}
\else\ifx\cvfont\cvbodylibertinus
  \newcommand{\cvsemi}[1]{{\rmfamily\fontseries{sb}\selectfont #1}}
\else
  % No real semibold: stroke the regular outline. Loaded here rather than with
  % the other packages so the default font pairing does not carry pdfrender.
  \usepackage{pdfrender}
  \newcommand{\melinewidth}{0.18pt}
  \newcommand{\cvsemi}[1]{%
    \textpdfrender{TextRenderingMode=FillStroke,LineWidth=\melinewidth}{#1}}
\fi\fi
...
\else
  ...
  \pagestyle{empty}   % a one-page document does not need to say it is page 1
\fi\fi
...
% Level 3: same face, medium weight, italic -- audible under a bold level 2
% without competing with it. Used for course audience and service groupings.
\titleformat{\subsubsection}{\cvheadfamily\normalsize\itshape}{}{0pt}{\cvsemi}

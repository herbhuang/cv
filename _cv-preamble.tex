...
% Optional \cvfont: garamond (default) or hyperlegible.
...
\ifx\cvmode\undefined
  \errmessage{CV mode not set. Compile cv-hhuang.tex, cv-hhuang.hyperlegible.tex, cv-hhuang.short.tex, or cv-hhuang-onepage.tex}
\fi
...
% Font theme. Unset means garamond, so the three length-entry files stay
% unchanged. hyperlegible is a parallel full CV, not a fourth density mode.
\ifx\cvfont\undefined
  \def\cvfont{garamond}
\fi
\newif\ifcvhyperlegible
\def\cvfontcheckgaramond{garamond}
\def\cvfontcheckhyperlegible{hyperlegible}
\ifx\cvfont\cvfontcheckhyperlegible
  \cvhyperlegibletrue
\else\ifx\cvfont\cvfontcheckgaramond
  \cvhyperlegiblefalse
\else
  \errmessage{Unknown \string\cvfont; use garamond or hyperlegible}
\fi\fi

% Encoding and production typography (pdflatex).
%
% garamond: EB Garamond carries the body, the title name, and own-name marks
% in reference entries (tabular lining figures: years and dates sit on the
% same baseline as the rest of the line). Alegreya Sans carries the three
% heading levels, using the family's native Bold and Medium Italic faces.
% Keep these defaults explicit: both packages set NFSS family defaults
% while loading, and package order should not silently change the finished CV.
%
% hyperlegible: Atkinson Hyperlegible Next for body, name, headings, and
% own-name marks. `t` is the atkinson package's tabular-lining option (not
% `tabular`). Pin both roman and sans to that family so \namefont's \rmfamily
% cannot fall back to Computer Modern. Needs the atkinson package dated
% 2025-03-14 or later, which is the Next family; TeX Live 2025+ on Overleaf.
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\ifcvhyperlegible
  \usepackage[sfdefault,t]{atkinson}
...
% First series in #1 that this family has at the current encoding/shape.
% Native names first: Alegreya Sans, EB Garamond, and Atkinson Hyperlegible
% Next all ship `medium`; `sb` is Alegreya's alias for that face (it has no
% SemiBold) and the real SemiBold in the other two. The fd must already be
% loaded -- \sffamily / \rmfamily / the document default all do that before
% we run.
\newcommand{\cvfontseries}[1]{%
  \def\cv@picked{}%
  \@for\cv@s:=#1\do{%
    \ifx\cv@picked\@empty
      \expandafter\ifx
        \csname\f@encoding/\f@family/\cv@s/\f@shape\endcsname\relax
      \else
        \edef\cv@picked{\cv@s}%
      \fi
    \fi}%
  \ifx\cv@picked\@empty
    \PackageWarning{cv}{No series in #1 for \f@family/\f@shape}%
  \else
    \expandafter\fontseries\expandafter{\cv@picked}%
  \fi
  \selectfont}

\makeatother

% Own-name emphasis in reference lists: the body family's SemiBold, else Medium.
\newcommand{\me}[1]{{\cvfontseries{sb,medium}#1}}
...
% Level 1: heading family Bold, uppercase and tracked.
\titleformat{\section}{\lsstyle\cvheadfamily\normalsize\bfseries\uppercase}{}{}{}%
  [{\cvsectionrule}]
% Level 2: heading family Bold, title case and lightly tracked.
\titleformat{\subsection}{\lsstyle\cvheadfamily\normalsize\bfseries}{}{}{}{}
% Level 3: heading family Medium Italic -- distinct without competing with L2.
% Request the native `medium` face; fall back to `sb` if Medium Italic is
% missing (Alegreya aliases `sb` to that same face).
\titleformat{\subsubsection}%
  {\cvheadfamily\normalsize\fontshape{\itdefault}\cvfontseries{medium,sb}}{}{0pt}{}

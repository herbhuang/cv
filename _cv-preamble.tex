...
% tablist labels are right-flushed to the tab stop: `2026` sits against the
% body; `Summer 2026` grows left into the page margin instead of shoving
% that row. Status that is not a date still belongs in the entry.
\ifcvextended
  % full
...
% Origin at the tab stop; \llap grows left into the page margin.
\SetLabelAlign{tabright}{\llap{#1\hskip 0.5em}}
\newlist{tablist}{description}{3}
\setlist[tablist]{
    leftmargin=\listtabwidth,
    labelwidth=0pt,
    labelsep=0pt,
    labelindent=\listtabwidth,
    align=tabright,
    style=unboxed,
    topsep=0em,
    partopsep=0em,
